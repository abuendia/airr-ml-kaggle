...
The project's dependencies can be installed from the current directory by running `pip install -e`. The 
project can also be run through docker container as:

## Code

Code for the solution is in [log-reg](./log-reg). To run the end-to-end workflow, run the bash script
[run.sh](./log-reg/run.sh). This includes the following modeling components:
